...
def val(fcn_model, epoch, args, criterion, val_loader, filename=None, filename1=None):
    ...
    for iter, (data, target) in tqdm.tqdm(
        ...
        input_print = inputs.cpu().numpy().reshape(c, h, w).transpose(1,2,0)
            ...
            #if args.file == True:
            #img_name  = "predictions_fcn_type/" + str(now.day) + "_" + str(now.hour) + "_" + str(now.minute) + "-" + str(epoch) + "-" + str(iter) + "-prediction.jpg"
        targ_name = "predictions/" + str(iter) + "-truth.jpg"
        inp_name  = "predictions/" + str(iter) + "-raw.jpg"
